...
public class ArrayTest {
	...
	public static void main(String[] args) {
		...
		// new 자료형[개수] 개수 만큼 연속된 변수를 생성하고 묶음으로 만든다.
		arr = new int[3];
		// 배열의 참조 : 입력, 읽기
		//배열이름 [index]: 배열의 변수는 순차적으로 위치하기 때문에 index값으로 참조한다.
		//index:0 부터 요소의 개수-1까지의  index를 가진다.
		...
		System.out.println("배열  arr2의 요소의 개수는 :"+arr2.length);
		...
		arr3=arr2;
		arr3=arr;
		System.out.println("배열  arr3의 요소의 개수는 :"+arr3.length);
		
		int[] arr4= {1,2,3,4,5};//  원본데이터, 주소값:0x100 
		int[] ref;
		ref=addAllArray(arr4,7);//addAllarry(0X100,7)=>0X100, ref=0X100;
        if(arr4==ref) {
	...
	//int 타입의 배열을 받고 ,증가시킬 정수 값을 받아서
	// 각 배열의 요소에 받은 정수 값 만큼 증가시키고 반환하는 메서드
	//전달 받은 배열을 반환
