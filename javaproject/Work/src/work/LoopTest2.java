...
//	1. int형 변수 x가 10보다 크고 20보다 작을 때 true인 조건식
		...
		System.out.println("x>10&&x<20=> " + b);
...
//	3. char형 변수 ch가 ‘x' 또는 ’X'일 때 true인 조건식
		...
		System.out.println("ch=='x'&&ch=='X'=>" + b);
...
//	8. 문자열 참조변수 str이 “yes”일 때 true인 조건식
		...
		boolean chk88 = str.equals("yes");// 저장하는 문자열비교 주소값을 저장
...
//	[4] 1+(-2)+3+(-4)+... 과 같은 식으로 계속 더해나갔을 때, 몇까지 더해야 총합이 100이상이되는지 구하시오.
		...
		while (true) {
			...
			sum4 = sum4 + num;
			if (sum >= 100) {
				System.out.println("1+(-2)+3+(-4)+... 과 같은 식으로 계속 더해나갔을 때, " + i + "번 더해야 총합이 100이상된다");
				break;
			}
...
// 주사위에서 발생 할 수있는 수의 범위 1~6
//두 개의 주사위의 합이 6이 되는 경우 dice1+dice2=6->dice2=6-dice1
		System.out.println("문제6*^*^*^*^*^*^*^*^*^*^*^*^*^*^*");
		for (i = 1; i <= 6; i++) {
			System.out.println("첫번째 주사위가 "+i+"일때,"+"두번째 주사위는 "+(6-i));
			
		}
		// [7] Math.random()을 이용해서 1부터 6사이의 임의의 정수를 변수 value에 저장하는 코드를완성하라. (1)에 알맞은 코드를
		// 넣으시오.
...
//
		str="12345";
		int cNum=Integer.parseInt(str); //자주사용
...
//
int answer = 5;//(int)(Math.random()*100)+1;
